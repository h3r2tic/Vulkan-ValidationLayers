...
ext_enabled extension_state_by_name(const ExtensionState &extensions, const char *extension_name) {
    if (!extension_name) return not_enabled;  // null strings specify nothing
    auto info = ExtensionState::get_info(extension_name);
    ext_enabled state =
        info.state ? extensions.*(info.state) : not_enabled;  // unknown extensions can't be enabled in extension struct
    return state;
}
