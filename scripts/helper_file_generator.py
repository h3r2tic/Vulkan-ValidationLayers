...
class HelperFileOutputGenerator(OutputGenerator):
    ...
    def GenerateExtensionHelperHeader(self):
        ...
        for type in ['Instance', 'Device']:
            ...
            struct  = [struct_decl]
            struct.extend([ '    ext_enabled vk_feature_version_1_1{not_enabled};'])
            struct.extend([ '    ext_enabled %s{not_enabled};' % field_name[ext_name] for ext_name, info in extension_items])
            ...
            struct.extend([
                '',
                '    struct %s {' % req_type,
                '        const ext_enabled %s::* enabled;' % struct_type,
                '        const char *name;',
                '    };',
                '    typedef std::vector<%s> %s;' % (req_type, req_vec_type),
                '    struct %s {' % info_type,
                '       %s(ext_enabled %s::* state_, const %s requires_): state(state_), requires(requires_) {}' % ( info_type, struct_type, req_vec_type),
                '       ext_enabled %s::* state;' % struct_type,
